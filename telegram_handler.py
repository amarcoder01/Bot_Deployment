...
class TelegramHandler:
    ...
    def __init__(self):
        ...
        self.advanced_qlib = AdvancedQlibStrategies()
        ...
        self.deep_learning = DeepLearningService()
...
⚠️ **Disclaimer:** This is technical analysis only. Always do your own research and consider risk management.
    ...
    async def _get_ai_sentiment_analysis(self, symbol: str) -> dict:
        ...
        try:
            ...
            return {'overall_score': 0, 'label': 'Neutral', 'confidence': 50}
    
    async def _analyze_news_sentiment(self, symbol: str) -> float:
        """Analyze news sentiment using AI"""
        try:
            # Get recent news
            news_data = await self.market_service.get_market_news(symbol, limit=10)
            ...
            if len(news_text) > 50:
                sentiment_prompt = f"Analyze the sentiment of this financial news about {symbol}. Return a score between -1 (very bearish) and 1 (very bullish): {news_text[:1000]}"
    ...
    def _calculate_price_targets(self, df: pd.DataFrame, indicators: dict) -> dict:
...
• **Risk Level:** {'High' if volatility_level == 'High' else 'Medium' if volatility_level == 'Medium' else 'Low'}"""
    ...
    async def run(self) -> None:
        ...
        try:
            ...
            # Start alert monitoring since job queue is disabled
            logger.info("Starting alert monitoring...")
            alert_monitoring_task = asyncio.create_task(self.alert_service.start_alert_monitoring())
            ...
            try:
                ...
                self.alert_service.stop_alert_monitoring()
